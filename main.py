...
# Streamlit 페이지 설정
st.set_page_config(page_title="✨ MBTI로 찾는 꿈의 직업! ✨", page_icon="🌟", layout="wide")
...
st.markdown("""
    ...
    @keyframes glow {
        from { text-shadow: 0 0 10px #fff, 0 0 20px #ff6b6b, 0 0 30px #ff8e53; }
        to { text-shadow: 0 0 20px #fff, 0 0 30px #ff9a9e, 0 0 40px #fad0c4; }
    }
    .subtitle {
        font-size: 1.8em;
        color: #fefefe;
...
mbti_jobs = {
    ...
    "INTP": {
        ...
        "descriptions": {
            ...
            "프로그래머": "코드를 통해 아이디어를 현실로 만듭니다. 창의적 문제 해결! 💾",
    ...
    "ENTJ": {
        ...
        "descriptions": {
            ...
            "변호사": "법률 문제 해결과 정의 구현! 논리와 설득력의 달인! ⚖️",
    ...
    "ENTP": {
        ...
        "descriptions": {
            ...
            "저널리스트": "이야기를 발굴하고 세상에 전파해요. 호기심의 전령! 📰",
    ...
    "INFJ": {
        ...
        "descriptions": {
            ...
            "작가": "글로 감동과 메시지를 전달해요. 상상력의 창조자! 📖",
            "사회복지사": "사회적 약자를 돕고 더 나은 세상을 만들어요! 🌍",
    ...
    "ENFP": {
        ...
        "descriptions": {
            ...
            "여행 가이드": "사람들에게 새로운 경험을 선사해요. 모험의 안내자! 🌴",
    ...
    "ESFJ": {
        ...
        "descriptions": {
            ...
            "판매원": "고객과 소통하며 제품을 판매해요. 친화력의 달인! 🛍️",
...
if st.button("🎉 직업 추천받기! 🎉"):
    # 추가 화려한 효과
    with st.spinner("🚀 당신의 꿈의 직업을 찾는 중..."):
        time.sleep(1)
        st.balloons()
    ...
    for job in mbti_jobs[mbti]["jobs"]:
        # 직업을 클릭하면 세션 상태 업데이트
        if st.button(f"{job} <span class='emoji'>✨</span>", key=job, help="클릭하면 직업 설명을 볼 수 있어요!"):
...
with st.sidebar:
    ...
    if st.session_state.selected_job:
        for mbti_type in mbti_jobs:
            if st.session_state.selected_job in mbti_jobs[mbti_type]["jobs"]:
                description = mbti_jobs[mbti_type]["descriptions"].get(st.session_state.selected_job, "설명 준비 중이에요! 😊")
